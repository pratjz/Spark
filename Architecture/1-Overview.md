...
The code runs somewhat like the following figure:
...
The actual execution procedure is more complicated than what we discussed above. Spark first creates a logical plan for each application, then it transforms the logical plan into a physical plan (a DAG graph of map/reduce stages and map/reduce tasks). After that, concrete map/reduce tasks will be lanuched to process the input data. Below is the details of the logical plan of this application:
